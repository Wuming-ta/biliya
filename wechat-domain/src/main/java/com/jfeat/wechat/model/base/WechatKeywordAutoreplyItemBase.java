/*
 *   Copyright (C) 2014-2017 www.kequandian.net
 *
 *    The program may be used and/or copied only with the written permission
 *    from www.kequandian.net or in accordance with the terms and
 *    conditions stipulated in the agreement/contract under which the program
 *    has been supplied.
 *
 *    All rights reserved.
 *
 */

/*
 * This file is automatically generated by tools.
 * It defines fields related to the database table.
 *
 * DON'T EDIT IT. OTHERWIDE IT WILL BE OVERRIDE WHEN RE-GENERATING IF TABLE CHANGE.
 */
package com.jfeat.wechat.model.base;

import com.jfinal.plugin.activerecord.IBean;
import com.jfeat.core.BaseModel;
import java.math.BigDecimal;
import java.util.Date;

public abstract class WechatKeywordAutoreplyItemBase<M extends WechatKeywordAutoreplyItemBase<?>> extends BaseModel<M> implements IBean {

    /**
     * Table fields 
     */
    public enum Fields {
        ID("id"),
        KEYWORD_ID("keyword_id"),
        TYPE("type"),
        TITLE("title"),
        DIGEST("digest"),
        CREATE_TIME("create_time"),
        CONTENT("content"),
        URL("url"),
        THUMB_URL("thumb_url"),
        ENABLED("enabled");
        
        private String name;
        Fields(String name) {
            this.name = name;
        }

        public String toString() {
            return name;
        }
        public String like(Object obj) {
            return new StringBuilder(this.toString()).append(" LIKE ").append(obj).toString();
        }
        public String eq(Object obj) {
            return new StringBuilder(this.toString()).append("=").append(obj).toString();
        }
        public String ge(Object obj) {
            return new StringBuilder(this.toString()).append(">=").append(obj).toString();
        }
        public String lt(Object obj) {
            return new StringBuilder(this.toString()).append("<").append(obj).toString();
        }
        public String gt(Object obj) {
            return new StringBuilder(this.toString()).append(">").append(obj).toString();
        }
        public String le(Object obj) {
            return new StringBuilder(this.toString()).append("<=").append(obj).toString();
        }
        public String isNull() {
            return new StringBuilder(this.toString()).append(" IS NULL").toString();
        }
        public String notNull() {
            return new StringBuilder(this.toString()).append(" NOT NULL").toString();
        }
        public String notEquals(Object obj) {
            return new StringBuilder(this.toString()).append("<>").append(obj).toString();
        }
    }

    public void setId(Integer var) {
        set(Fields.ID.toString(), var);
    }

    public Integer getId() {
        return (Integer) get(Fields.ID.toString());
    }

    public void setKeywordId(Integer var) {
        set(Fields.KEYWORD_ID.toString(), var);
    }

    public Integer getKeywordId() {
        return (Integer) get(Fields.KEYWORD_ID.toString());
    }

    public void setType(String var) {
        set(Fields.TYPE.toString(), var);
    }

    public String getType() {
        return (String) get(Fields.TYPE.toString());
    }

    public void setTitle(String var) {
        set(Fields.TITLE.toString(), var);
    }

    public String getTitle() {
        return (String) get(Fields.TITLE.toString());
    }

    public void setDigest(String var) {
        set(Fields.DIGEST.toString(), var);
    }

    public String getDigest() {
        return (String) get(Fields.DIGEST.toString());
    }

    public void setCreateTime(Date var) {
        set(Fields.CREATE_TIME.toString(), var);
    }

    public Date getCreateTime() {
        return (Date) get(Fields.CREATE_TIME.toString());
    }

    public void setContent(String var) {
        set(Fields.CONTENT.toString(), var);
    }

    public String getContent() {
        return (String) get(Fields.CONTENT.toString());
    }

    public void setUrl(String var) {
        set(Fields.URL.toString(), var);
    }

    public String getUrl() {
        return (String) get(Fields.URL.toString());
    }

    public void setThumbUrl(String var) {
        set(Fields.THUMB_URL.toString(), var);
    }

    public String getThumbUrl() {
        return (String) get(Fields.THUMB_URL.toString());
    }

    public void setEnabled(Integer var) {
        set(Fields.ENABLED.toString(), var);
    }

    public Integer getEnabled() {
        return (Integer) get(Fields.ENABLED.toString());
    }


    ///////////////////////////////////////////////////////////////////////////
    
}
